...
export function createLabViewer(host: HTMLElement, callbacks: {
  ...
  scene.add(new THREE.HemisphereLight('#e3f2ff', '#283643', 2.5));
  const key = new THREE.DirectionalLight('#fff2db', 3); key.position.set(4, 7, 5); scene.add(key);
  const fill = new THREE.DirectionalLight('#7cb6ff', 1.7); fill.position.set(-5, 4, -5); scene.add(fill);
  ...
  function setConfig(next: LabConfig) {
    ...
    next.references.forEach((sensor, i) => {
      ...
      addMarker({ origin: sensor.position, mountPart }, { signal: 'x', channel: i }, sensor.name, '#f8c26e');
    });
  ...
  return {
    ...
    render(time: number, selected: LabSelection, drives: number[], sourceValues: number[]) {
      ...
      amount += ((exploded ? 1 : 0) - amount) * (1 - Math.exp(-dt * 5));
    ...
    dispose() { resize.disconnect(); controls.dispose(); model?.dispose(); clear(markers); clear(paths); clear(waves); clear(field); clear(stripes); ground.geometry.dispose(); ground.material.dispose(); renderer.dispose(); markerRows.forEach(row => row.button.remove()); renderer.domElement.remove(); },
  };
}
