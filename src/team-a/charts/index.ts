...
function plot(canvas: HTMLCanvasElement, series: { values: number[]; color: string }[], min: number, max: number, left: string, right: string) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    ctx.fillStyle = '#819aa7'; ctx.fillText((max - (max - min) * i / 3).toFixed(1), 1, y + 3);
  }
...
export function drawWaveform(canvas: HTMLCanvasElement, result: RunResult, end: number, selected: { signal: SignalKind; channel: Corner }) {
  ...
  const series = [{ values: Array.from(signal.subarray(start, end)), color: colors[0] }];
  if (selected.signal === 'd' || selected.signal === 'e') series.unshift({ values: Array.from(result.signals.d[index].subarray(start, end)), color: '#617984' });
  let peak = 0.001;
...
export function drawConvergence(canvas: HTMLCanvasElement, curves: number[][]) {
