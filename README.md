...
目标是四类双排SUV的交互式RNC展示、调试与试听应用。当前lab-v3包含ICE/BEV/HEV/EREV、参考改制、四门扬声器启禁、FxLMS调参、信号图及空间SPL采样；所有源/路径为**合成教学模型**，当前仍是**先计算16秒再回放**，持续实时运行尚未完成。

当前为**完整目标开发基线，尚非最终交付**。源码/证据通过[PR #7](https://github.com/KnowCooker/RNC_3D_SIMULATION/pull/7)交接，开放PR不等于main已合并。[本轮验证](docs/evidence/full-lab/iteration-01/README.md)包括38项测试与90项整页断言；模型精细度、剖面质量、实时运行及新版长稳/设备/离线/正式包仍有缺口。旧demo-v2的20分钟与离线验证不能代表新版通过。
...
1. 选择车型、工况和算法参数，点击“计算当前实验”；当前模式明确为16秒预计算回放。
2. 旋转、透明、连续拆解或选择剖面；点Q/REF/OUT/MIC或原理图通道查看q/x/u/d/a/e和频谱。
3. 开启改制，在可见结构上加参考或删除所选参考，启禁门扬声器；这些变化要求重新计算，误差点固定。
4. 播放并切换原声/残余、座位、场与切片、波前和路径；八路d/e共用防削波衰减系数，物理分析使用未改动数据。
5. 参数、拓扑改变后旧结果/旧声场清空，计算成功才继续。信号、场、图表读取统一回放时间。波前/传播线是教学示意。
...
旧demo-v2候选包验证代码为`838cdf3`，由A1 `45fe841`及A2 `2760f43`组成，见[历史本机交付验收](docs/evidence/A1/A1-004/README.md)。lab-v3已通过生产构建，尚未形成通过完整验收的新版分发包；不能把旧包或单角色分支工件当成新版完整交付。
