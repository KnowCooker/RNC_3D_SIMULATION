...
Range Rover 外观资产现在在道路、车间复用**同一个加载实例**。车间仅列出原 GLB 中能按名称与真实网格共同定位的 10 个总成：前/后保险杠、前舱盖、左右前门及后视镜、尾门及后窗、车顶、方向盘、座舱内饰整体、排气整体。GLB 的 171 个 mesh 只是材质切分，并非 171 个零件；车轮橡胶/刹车等网格横跨多个轮位，后侧围网格也不是各后门，故这些暂不标为独立可拆件。10 组均把原始几何移入自身三维变换组，支持单件、顺序自动拆解、暂停、逆序回装及场景切换保留状态。BEV 资产尚无可核实分组；HEV/EREV 仍只是 Range Rover 外观参考，均不展示这 10 组为本车型真实结构。

[开发页实测脚本](qa-real-parts.playwright-cli.js)和[结果](real-parts-browser-result.json)检查：源资产 171 mesh 在分组后仍全数存在，171份几何与30份材质均恰好释放一次；10 组各有非空真实子网格，拆装位移为 0.36～0.90 m。道路→车间时同一外观，前舱盖与左前门拆出后状态为2/10，往返道路仍为2/10，手动逆序与自动回装到0/10。写实模式物理标记仍隐藏，页面异常/外网请求0；360px窗口拆装面板收起后可打开，HEV/EREV/BEV不显示这10组。实渲染原图：[道路后视](real-road-ice.png)、[车间合拢](real-workshop-assembled.png)、[前舱盖与车门拆出](real-workshop-two-parts.png)、[内饰整体移出](real-workshop-interior.png)、[窄屏车间](real-workshop-narrow.png)。[生产预览脚本](qa-real-parts-production.playwright-cli.js)重查同源GLB按需加载及跨场景拆件状态。

试把现有教学车的外壳/轮系隐藏、内部与声学点放进 Range Rover 时，浏览器[原始配准失败图](rejected-shell-overlay.png)显示教学头枕穿出车顶，教学仪表台压在前舱盖外，MIC不贴头枕；因此**未把两套资产简单叠加进产品**。目前写实资产的 10 组只完成**外观拆装**，不是完整机械/电驱结构；与四类教学模型、RNC物理锚点和算法尚未统一，仍不能按用户三图最终签收。下一步须按写实车尺寸制作并核验座椅/底盘/动力件、轮系和安装位，再由A1/集成协调者与B组同步车型布局契约和数值重验。
