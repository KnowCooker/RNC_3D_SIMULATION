...
Range Rover 外观资产现在在道路、车间复用**同一个加载实例**。车间列出原 GLB 中可核实的 30 个几何总成：前/后保险杠、前舱盖、左右前翼子板与灯组、前后轮区内侧板、左右前门、四个独立轮端、尾门、左右后灯、后侧围、门槛、车顶、车窗玻璃、机舱动力网格、方向盘、左右前排带头枕座椅、带三个头枕的后排长椅、座舱其余内饰、车身/底盘主体网格和排气。它们是**这份美术资产的可辨几何分组**，不是 30 个经原厂零件目录核对的机械总成。GLB 原始 171 个 mesh 主要按材质切分，不是 171 个零件。轮胎、制动盘、卡钳虽各合为一个 mesh，但逐三角形审计发现四个空间分离的轮位：每轮分别有 360、256、148 个三角形，没有三角形跨越前后/左右中线；16 个轮辋 mesh 也能按真实空间位置各归 4 个。加载器保留原顶点属性，把三张合并网格的三角形按轮位重组，四个轮端各含 7 个原资产来源网格，不复制单轮几何，也不增加三角形。内饰 `Object_88` 的 4,024 个三角形按**连通几何岛及实际三维包围盒**划分为左前座 636、右前座 630、后排长椅 498、其余内饰 2,260 个；原材质、法线、UV 和三角形总数不变。后排是一个长椅总成，不假称三个独立座椅机构。机舱动力网格按 `Chassis6` 父节点与 `rM_Engine_Max1` 材质双重归属提取，避免误带前舱盖/尾门中同材质的网格。后侧围仍不是可核实的各后门。30 组支持单件、顺序自动拆解、暂停、逆序回装及场景切换保留状态。BEV 资产尚无可核实分组；HEV/EREV 仍只是 Range Rover 外观参考，均不展示这 30 组为本车型真实结构。

[开发页实测脚本](qa-real-parts.playwright-cli.js)和[结果](real-parts-browser-result.json)检查：原资产 171 mesh 在轮系和座椅拆分后为 183 个渲染 mesh，仍为 74,127 个三角形；运行中的 183 份几何和 30 份材质均恰好释放一次。30 组各有非空真实子网格，四个轮端各 7 个、三个座椅组各 1 个。道路→车间时同一外观，前舱盖与左前门拆出后状态为2/30，往返道路仍为2/30，手动逆序与自动回装到0/30；三组座椅分别拆出达3/30、四轮分别拆出达4/30，均可复位；完整自动拆解到30/30再逆序回装到0/30。写实模式物理标记仍隐藏，页面异常/外网请求0；360px窗口拆装面板收起后可打开，HEV/EREV/BEV不显示这30组。实渲染原图：[道路后视](real-road-ice.png)、[车间合拢](real-workshop-assembled.png)、[前舱盖与车门拆出](real-workshop-two-parts.png)、[机舱网格拆出](real-workshop-engine-bay.png)、[三组座椅拆出](real-workshop-three-seats.png)、[四轮端拆出](real-workshop-four-wheels.png)、[30组拆出](real-workshop-all-parts.png)、[完整回装](real-workshop-restored.png)、[内饰其余部分移出](real-workshop-interior.png)、[窄屏车间](real-workshop-narrow.png)。[生产预览脚本](qa-real-parts-production.playwright-cli.js)重查同源GLB按需加载及跨场景拆件状态。

试把现有教学车的外壳/轮系隐藏、内部与声学点放进 Range Rover 时，浏览器[原始配准失败图](rejected-shell-overlay.png)显示教学头枕穿出车顶，教学仪表台压在前舱盖外，MIC不贴头枕；因此**未把两套资产简单叠加进产品**。本批改为从该写实车**自身的座舱网格**中取两张前座及后排长椅，排除了中控台/顶棚碎片，实渲染可见座椅带头枕并能独立于剩余内饰拆出；这仍只是美术形面归属，后排没有三个独立座椅机构，也未提供座椅滑轨或安全约束精度。JLR 的[2014年原厂发布资料](https://media.jlr.com/corporate/news/2014/08/pebble-beach-debut-new-range-rover-sport-svr-fastest-most-powerful-land-rover-ever)与[车型规格册](https://www.landrover.co.uk/content/dam/lrdx/pdfs/uk/brochures/misc/RANGE-ROVER-SPORT-SVR_tcm295-140483.pdf)确认该代 SVR 为纵置 5.0 升机械增压 V8、ZF 8HP70 八速自动变速箱、四轮驱动；**原 GLB 并非原厂 CAD**，本批只把其中可见的机舱动力表面单列，未核实其缸体、变速箱、传动轴、悬架的精确构形。当前写实资产的 30 组不是完整机械/电驱结构；与四类教学模型、RNC物理锚点和算法尚未统一，仍不能按用户三图最终签收。下一步须按写实车尺寸制作并核验底盘/传动/悬架件及安装位，再由A1/集成协调者与B组同步车型布局契约和数值重验。

[同车坐标复测脚本](audit-ice-layout.playwright-cli.js)与[原始 JSON](ice-layout-audit.json)按4.8米车长归一化后从本批真网格测得：前轮中心约 `(±0.815, 0.372, 1.514)` m，后轮中心约 `(±0.815, 0.372, -1.325)` m；前排头枕上部包围盒中心约 `(±0.457, 1.444, -0.368)` m，后排左右头枕约 `(±0.429, 1.416, -1.272)` m。后者只是**几何中心候选**，尚非麦克风安装点。现[共享物理契约](../../../../src/shared/lab-contracts.ts)前排 MIC 为 `(±0.48, 1.65, 0.4)` m，比写实头枕中心高约0.206 m、朝车头偏约0.768 m；后排 MIC 比写实头枕中心高约0.234 m、朝车头偏约0.262 m。因此目前写实模式继续隐藏物理标记，而不能只在A2画面挪标签。下一步由A1/集成协调者与B组建立**按车型版本化的布局契约**，在同一布局里同时给出头枕麦克风、四门扬声器、轮端噪声源和参考传感器坐标，并重验B组路径与声场。现有前门外形可测，GLB尚无独立扬声器网格或可验证的后门分件，四门扬声器仍需按该车门板制作并核验。
