async page => {
  ...
  for (const kind of ['ice', 'bev', 'hev', 'erev']) for (const [view, camera, target, body] of views) {
    if (['front-seat', 'rear-seat', 'door-close'].includes(view) && kind !== 'bev') continue;
    const state = await page.evaluate(({ kind, view, camera, target, body }) => {
      ...
      return { kind, view, body, camera: qa.camera.position.toArray(), target: qa.controls.target.toArray(), triangles, meshes,
