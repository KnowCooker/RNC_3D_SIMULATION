...
2026-09-24：`tests/a2-vehicle-model.test.ts` 的 2 项测试通过，覆盖四类架构的必需/禁用部件、五座、轮坐标、四扬声器坐标、独立爆炸层级、有限几何、尺寸范围与几何/材质/纹理幂等释放；类型检查及 A/B 边界检查通过。本轮加入弯曲车门蒙皮、车头收肩曲面、实际连接的保险杠/灯组、双拼座椅/轮廓缝线/程序化织物微表面、金属车漆环境反射，适当降低弹簧管线细分后，模型三角数为 ICE 43,668、BEV 44,136、HEV 46,056、EREV 46,460。不等同实际帧率承诺。
...
本轮进一步完成 X/Y/Z 三方向逐部件实际网格剖面、孔洞保留、爆炸与截面的世界位置一致性检查，并回归 13 个安装点、32 条路径、32 个路径光点、12 个波前的附着位置。独立渲染的截图、采样条件与限制见 [A2-FULL-002](../evidence/A2/A2-FULL-002/README.md)，剖面算法与可支持几何的范围见 [剖面说明](section-rendering.md)。车门和车顶等蒙皮添加的 6–8 mm 厚度是为了教学剖面可见性采用的显示网格厚度，不代表原厂钣金厚度，不能用于结构有限元分析。

## 尚未达到的完整目标

这是有资料支撑的程序化详细教学模型，**尚未达到《地平线》系列的外观精度**。尚无扫描车身、完整 PBR 纹理资产集、焊缝/紧固件/完整内饰缝线、生产级悬架运动学、散热软管和真实线束布置；现有织物纹理与轮廓缝线只是局部精细化。不能用“四类可切换”或自动测试通过替代视觉精度验收；后续需逐视角检查并迭代曲面和细节，必要时引入有明确授权的精细资产。本模型不提供真实车辆 NVH 标定，也不把几何爆炸位置用于重建物理通道。
