...
集成协调者已对当前完整页面补充[1080p渲染基线](../evidence/full-lab/current-performance/README.md)：RTX4070上六种状态无异常，但实际绘图缓冲为1588×618，未按1280×720等效像素预算限制；rAF读数不算目标核显帧率。A2下一批从 fork/main 开始时先核对这份脚本及原始数据，在 viewer 内处理渲染分辨率/画质与用户可达视角，并在指定核显复测；A1继续负责页面与真实信号联动，不代写 viewer。
